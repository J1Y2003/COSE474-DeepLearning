...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        window_shape = (in_ch_size, self.W.shape[2], self.W.shape[3])
        windows = [view_as_windows(sample, window_shape, step=1) for sample in x]
        windows = torch.stack(windows)
        W_expanded = self.W.unsqueeze(0).unsqueeze(2).unsqueeze(3) #(1, 10, 1, 1, 1, 3, 3)
        output = (windows * W_expanded).sum(dim=(-1, -2, -3)) + self.b #(64, 10, 26, 26)
        return output
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        ###################################
        batch_size, in_ch_size, height, width = x.shape
        window_shape = (in_ch_size, self.pool_size, self.pool_size)
        windows = [view_as_windows(sample, window_shape, step=2) for sample in x]
        windows = torch.stack(windows)
        result = windows.amax(dim=(-1, -2))
        output = result.permute(0, 4, 1, 2, 3).squeeze(2)
        return output
